...
void heap_destruir(heap_t *heap, void destruir_elemento(void *e)){
	...
	while (!heap_esta_vacio(heap)){
		void * elemento_del = heap_desencolar(heap);
		if (destruir_elemento) destruir_elemento(elemento_del);
	}
...
bool heap_encolar(heap_t *heap, void *elem){
	...
	int v_tam =  (int)vector_obtener_tamanio(heap->vec);
	int ocup = (OCUPADO*v_tam/100);
	if (pos >= ocup){
		vector_redimensionar(heap->vec, (size_t)v_tam*DUP); 
		for(size_t i= heap->cantidad+1; i< vector_obtener_tamanio(heap->vec); i++){
			vector_guardar(heap->vec,i,NULL); //inicializa
		}
	}
...
void downheap(vector_t* vector, size_t cant, size_t pos, cmp_func_t cmp){
	...
	if (dato1) {
		if ( dato2 && (izq_pos <= cant) && (cmp(dato1,dato2) < 0)) 
	 		max = izq_pos;
	 	dato1 = vector_obtener(vector, max);
 		if ( dato3 && (der_pos <= cant) && (cmp(dato1,dato3) < 0)) 
 			max = der_pos;
		if (max != pos) {
			vector_swap(vector, pos, max);
...
void *heap_desencolar(heap_t *heap){
	...
	if ((heap->cantidad > TAM_INICIAL) && (heap->cantidad <= desocupado)){
		vector_redimensionar(heap->vec, (size_t)v_tam/DIV);	
	}
